...
平台按以下条件处理，并且只有镜头规格完全一致时才允许套用白色款价格：

- IPC2 / IPC3：白色款各价格层级 +2 EUR。
- 其他 IPC 系列（包括 IPC5 / IPC7）：不做 Black 加价。
- 非 IPC 系列（例如报警产品）：白色款各价格层级 +1 EUR。
- External Model 相同但存在多个镜头规格时，必须使用 Internal Model 中一致的镜头码
  （例如 `0280`、`0360`、`0600`、`2712`、`0832`）；无法唯一确认时禁止自动应用。
- 同一镜头约束也用于批量任务和 External Model 扩展索引的 France 价格锚点，缓存按
  `External Model + 镜头签名` 隔离，不会跨镜头复用价格。
